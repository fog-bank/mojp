        ...
        public static async Task<GetPDListResult> GetOrOpenPDLegalFile(bool forceCheck)
        {
            ...
            if (exists)
            {
                ...
                if (!DateTime.TryParseExact(
                    Settings.Default.PDListLastTimeUtc, "o", culture, DateTimeStyles.RoundtripKind, out lastCheckTime))
                {
                ...
                // 最終更新日時の取得
                if (!DateTime.TryParseExact(
                    Settings.Default.PDServerLastTimeUtc, "o", culture, DateTimeStyles.RoundtripKind, out lastModifiedTime))
                {
                    // PD S9 更新前時刻
            ...
            if (result == GetPDListResult.New || result == GetPDListResult.Update)
                Settings.Default.PDServerLastTimeUtc = lastModifiedTime.ToString("o", culture);
